<?php
/**
 * Fixed Comment Test Runner - URL Construction Fixed
 * This script runs all comment tests with proper URL construction
 */

require_once 'config/database.php';

echo "<h1>Fixed Comment System Test Runner</h1>";
echo "<p>This runner includes fixes for URL construction issues.</p>";
...
function build_api_url($endpoint) {
    ...
    // Get the script path and normalize it
    ...
    // Add script path if not empty
    if (!empty($script_path) && $script_path !== '/') {
        $url .= $script_path;
    }
    
    // Add final slash and endpoint
    $url .= '/' . $endpoint;
...
if (!$all_tables_exist || !$procedure_exists) {
    ...
    echo "<a href='final_comment_system_fix.php' style='display: inline-block; margin-right: 10px;'>";
    echo "<button style='background: #dc3545; color: white; padding: 10px 20px; border: none; cursor: pointer;'>Apply Complete Fix</button>";
    echo "</a>";
    echo "</div>";
...
// Run tests
echo "<h2>Running Fixed Tests</h2>";
...
if (mysqli_num_rows($news_result) > 0) {
    ...
    $test_data = [
        'news_id' => $news_id,
        'comment' => 'Test comment from fixed test suite - ' . date('Y-m-d H:i:s'),
        'parent_id' => null
    ];
...
if (mysqli_num_rows($parent_result) > 0) {
    ...
    $reply_data = [
        'news_id' => $news_id,
        'comment' => 'Test reply from fixed test suite - ' . date('Y-m-d H:i:s'),
        'parent_id' => $parent_id
    ];
...
echo "<a href='final_comment_system_fix.php' style='display: inline-block; margin-right: 10px;'>";
echo "<button style='background: #dc3545; color: white; padding: 12px 24px; border: none; cursor: pointer; font-size: 16px; font-weight: bold;'>Apply Complete Fix</button>";
echo "</a>";
...
echo "<p><small>This fixed test runner includes proper URL construction and better error handling.</small></p>";
?>

-- Affiliate Marketing System Database Update
-- Execute this in your PHPMyAdmin or MySQL console
...
CREATE TABLE IF NOT EXISTS `news_product_relations` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
...
INSERT INTO `affiliate_categories` (`name`, `slug`, `description`, `icon`, `sort_order`) VALUES
...
ALTER TABLE `affiliate_clicks` ADD CONSTRAINT `fk_affiliate_clicks_product` FOREIGN KEY (`product_id`) REFERENCES `affiliate_products` (`id`) ON DELETE CASCADE;
ALTER TABLE `news_product_relations` ADD CONSTRAINT `fk_news_product_relations_news` FOREIGN KEY (`news_id`) REFERENCES `news` (`id`) ON DELETE CASCADE;
ALTER TABLE `news_product_relations` ADD CONSTRAINT `fk_news_product_relations_product` FOREIGN KEY (`product_id`) REFERENCES `affiliate_products` (`id`) ON DELETE CASCADE;

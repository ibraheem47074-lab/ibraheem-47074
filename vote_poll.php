<?php
// Start output buffering to prevent any HTML/whitespace in JSON response
ob_start();
...
$column_check = mysqli_query($conn, "SHOW COLUMNS FROM poll_votes LIKE 'user_id'");
$has_user_id = mysqli_num_rows($column_check) > 0;

if ($has_user_id && $user_id) {
    // Check by both IP and user_id
    ...
    mysqli_stmt_bind_param($vote_check, 'is', $poll_id, $ip_address);
}
...
try {
    // Insert the vote - check if user_id column exists
    if ($has_user_id) {
        ...
        mysqli_stmt_bind_param($vote_insert, 'iis', $poll_id, $option_id, $ip_address);
    }
